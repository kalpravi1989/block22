...
const deleteParty = async (id) => {
  try {
    let response = await fetch(`${PARTIES_API_URL}/${id}`, {
        method: "DELETE",
    });
   let demo=await getAllParties();
   await renderParties(demo);
} catch (err) {
}
  ...
  try {
    ...
    parties.forEach((party) => {
      ...
      detailsButton.addEventListener('click', async (event) => {
        renderSinglePartyById(event.target.dataset.id);
...
// init function
const init = async () => {
 const demo=await getAllParties();
 await renderParties(demo);
